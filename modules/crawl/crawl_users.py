...
def save_to_mongo(dict_users,user_id):
    ...
    # if check_old_ID(user_id):
    #     # dict_users = crawl_users(signin_driver,user)
    #     collection_user.insert_one(dict_users)
    # else:
...
def scrape_data(driver,user_id, scan_list, section, elements_path, save_status):
    ...
    dict_users_keys = list(dict_users.keys())
    if save_status == 4:
        page.append(user_id)
    ...
    for i, _ in enumerate(scan_list):
        ...
        data = driver.find_elements_by_xpath(elements_path[i]+"//span")
        
        dict_users[dict_users_keys[i]] = "\n".join(
                [d.text for d in data]
            )
        
    print("\n\n{}\n\n".format(dict_users))
    save_to_mongo(dict_users,user_id.split("/")[-1])
...
def scrap_profile(driver,user_id):
    ...
    scrape_data(driver,"https://www.facebook.com/"+user_id, scan_list, section, elements_path, save_status)
...
def crawl_users(driver,user_id):
    ...
    if user_id:
        print("\nStarting Scraping User Profile...")

        # login('nuemotiononetwo@gmail.com', '113113113')
        scrap_profile(driver,user_id)
        # driver.close()
    else:
